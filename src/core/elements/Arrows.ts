...
export default class Arrows extends BaseElement implements Drawable {
  ...
  public draw (props: any): void {
    const p = this.dimensions[1] / 20 - 1
    const q = 1
    ...
    this.ctx.lineTo(...this.filters(this.dimensions[1] / 20, 0))
    this.ctx.lineTo(...this.filters(p, -q))

    this.ctx.moveTo(...this.filters(q, p))
    this.ctx.lineTo(...this.filters(0, this.dimensions[1] / 20))
    this.ctx.lineTo(...this.filters(-q, p))
    //
    this.ctx.moveTo(...this.filters(0, -this.dimensions[1] / 20))
    this.ctx.lineTo(...this.filters(0, this.dimensions[1] / 20))
    //
    this.ctx.moveTo(...this.filters(-this.dimensions[1] / 20, 0))
    this.ctx.lineTo(...this.filters(this.dimensions[1] / 20, 0))
    this.ctx.stroke()
  }
}
